...
int c_speed = 130;//THis is for the speed increment and decrement.
int b_speed = 90;//This is the constant speed for reversing.
...
void blinking(){
 ...
 void turningRight(){
 for(int angle = 42; angle >=14; angle -= 2) // goes from 45 degrees to 90 degrees
 { // in steps of 1 degree
...
void meanPosR(){
  for(int angle = 14; angle <= 42; angle += 2)
 {
 agv_servo.write(angle); 
 delay(50);
 }
...
//=======================================
 ...
 void servo_right(){
 for(int angle = 20; angle <=150; angle += 1)
 {
 IR_servo.write(angle); 
 delay(8);
 }
...
//========================================================
 if(obstacle_right > obstacle_left && obstacle_right>60)
    {
               turningRight();
               movingForward(c_speed);
               delay(1000);
...
//               meanPosL();
        ...
        else if(obstacle_right < obstacle_left && obstacle_left>60){
              turningLeft();
              movingForward(c_speed);
              delay(1000);
              meanPosL();
...
//              meanPosR();
              movingForward(c_speed);
  } else if (obstacle_left<=60 || obstacle_right<=60){
...
//=============
obstacle = IR_sensor();
Serial.print("Initial Distance Checkup : ");
Serial.println(obstacle);
};
void loop(){
//   buttonAssignment();
//  switchCase(); //The code for the IR reciever
// IR_Receiver_Module();//Code for the ideal ultrasonic sensor.
...
//Beginning of Obstacle Avoidance
//START OF BODY
//==============
 if(obstacle>60){
   movingForward(c_speed);
    ...
    buzzer_action();//This logic function should kick in.
  };
